...
from vanilla.abci.types_pb2 import Response, ResponseInfo, OK, ResponseQuery
...
class SApp(BaseApplication):
    ...
    def query(self, reqQuery):
        return ResponseQuery(key=reqQuery.data, value=b'dave', proof=b'0x12')
...
def test_handler():
    ...
    res, err  = read_message(BytesIO(data), Response)
    assert res
